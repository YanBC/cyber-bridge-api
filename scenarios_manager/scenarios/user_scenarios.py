...
from srunner.scenariomanager.scenarioatomics.atomic_behaviors import (ActorTransformSetter,
                                                                      ...
                                                                      WaypointFollower,
                                                                      LaneChange)
from srunner.scenariomanager.scenarioatomics.atomic_criteria import (CollisionTest, KeepLaneTest)
...
class SpeedLimit(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        self.timeout = timeout
        self._map = CarlaDataProvider.get_map()
        self.max_speed_limit = 20/3.6 # 20km/h
        self.traffic_sign_location = carla.Location(-100, 5)
        # get direction from config name
        self._config = config
        self._direction = None
        self._transform_visible = None
        ...
        # print("landmark list{}".format(landmarks))
        # for landmark in landmarks:
        #     if "Sign_274" in landmark:
        #         self.traffic_sign_location = landmark.transform.lcation
        #         break
    ...
    def _create_behavior(self):
        ...
        trigger_velocity = TriggerVelocity(self.ego_vehicles[0], 1.75*self.max_speed_limit)
        ...
        # end condition
        self.ref_wp = CarlaDataProvider.get_map().get_waypoint(self.traffic_sign_location)
    ...
    def _create_test_criteria(self):
        ...
        speed_criterion = MaxSpeedLimitTest(self.ego_vehicles[0], self.max_speed_limit)
        location_criterion = ArriveAtLocationTest(self.ego_vehicles[0], self.ref_wp.transform.location)
...
class StopSign(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        self._map = CarlaDataProvider.get_map()
        self.speed_require = 30/3.6 # 20km/h
        self.stop_sign_location = carla.Location(-10, 5)
        self.ref_wp = CarlaDataProvider.get_map().get_waypoint(self.traffic_sign_location)
        # get direction from config name
        self._config = config
        self._direction = None
    ...
    def _create_test_criteria(self):
        ...
        vehicle_run_criterion = VehicleRunTest(self.ego_vehicles[0])
        stand_still_criterion = StandStillTimeTest(self.ego_vehicles[0], duration = 3)
        distance_criterion = ArriveAtLocationTest(self.ego_vehicles[0], self.ref_wp.transform.location, 2, False)
...
class WalkerThroughCrosswalk(BasicScenario):
    ...
    """    
    The ego vehicle is passing through a road,
    And encounters a pedestrian crossing the road. it will stop unit the cyclist/pedestrian walks away.
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False,
        ...
        self._time_to_reach = 4.5        
        self._walker_yaw = 0
        self.wp_walker = self._wmap.get_waypoint(carla.Location(x=152.5, y=50),
        ...
        super(WalkerThroughCrosswalk, self).__init__("WalkerThroughCrosswalk",
                                                    ego_vehicles,
                                                    config,
                                                    world,
                                                    debug_mode,
    ...
    def _spawn_adversary(self, transform):
        ...
        self._other_actor_target_velocity = 6.5/3.6 # 6.5km/h
        walker = CarlaDataProvider.request_new_actor('walker.*', transform)
        adversary = walker
    ...
    def _obtain_left_most_lane_wp(self, waypoint):       
        ...
        for _ in range(100):
            ...
            if wp_next is not None:
                if wp_next.lane_type == carla.LaneType.Sidewalk or wp_next.lane_type == carla.LaneType.Shoulder:
                    return wp_next
                    #break                
                print("type{}, location{}".format(wp_next.lane_type, wp_next.transform.location))
                wp_dir = wp_next.transform.get_forward_vector()
                if wp_dir.x * ref_dir.x + wp_dir.y * ref_dir.y < 0: # different director
    ...
    def _initialize_actors(self, config):
        ...
        Custom initialization
        """        
        waypoint = self.wp_walker # self._obtain_left_most_lane_wp(self.wp_walker)
        walker = self._spawn_adversary(waypoint.transform)

        position_yaw = waypoint.transform.rotation.yaw
        lane_width = self.wp_walker.lane_width
        offset_location = carla.Location(lane_width * math.cos(math.radians(position_yaw)),
    ...
    def _create_behavior(self):
        ...
        root = py_trees.composites.Parallel(
            policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE, name="WalkerThroughCrossWalker")

        start_condition = InTimeToArrivalToLocation(self.ego_vehicles[0], 4.5+2, self._location_of_collision)
    ...
    def _create_test_criteria(self):
        ...
        collision_criterion = CollisionTest(self.ego_vehicles[0])
        vehicle_run_criterion = VehicleRunTest(self.ego_vehicles[0])
        stand_still_criterion = StandStillTimeTest(self.ego_vehicles[0], self.other_actors[0], duration = 3)
...
class CrossTrafficRedLight(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        # for landmark in landmarks:
        #     if "Sign_274" in landmark:
        #         self.traffic_light_location = landmark.transform.lcation
        #         break    

        super(CrossTrafficRedLight, self).__init__("CrossTrafficRedLight",
                                        ego_vehicles,
                                        config,
                                        world,
                                        debug_mode,
    ...
    def _retrieve_traffic_landmark(self,
            map: carla.Map,
    ...
    def _create_test_criteria(self):
        ...
        vehicle_run_criterion = VehicleRunTest(self.ego_vehicles[0])
        stand_still_criterion = StandStillTimeTest(self.ego_vehicles[0], duration = 3)
        distance_criterion = StandStillDistanceTest(self.ego_vehicles[0], self.ref_wp.transform.location, 2)
...
class CrossTrafficGreenLight(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        super(CrossTrafficGreenLight, self).__init__("CrossTrafficGreenLight",
                                        ego_vehicles,
                                        config,
                                        world,
                                        debug_mode,
