...
from srunner.scenariomanager.scenarioatomics.atomic_behaviors \
    import (AtomicBehavior, get_actor_control)
...
class DecelerateToVelocity(AtomicBehavior):
    ...
    The behavior will terminate, if the actor's velocity is at least
    target_velocity
    """

    def __init__(self, actor, brake_value, target_velocity,
                 name="Deceleration"):
        """
    ...
    def initialise(self):
        ...
        if self._type == 'walker':
            ...
            self._control.direction = CarlaDataProvider.get_transform(
                                        self._actor).get_forward_vector()

        super(DecelerateToVelocity, self).initialise()

    def update(self):
        """
        Set throttle to throttle_value, as long as velocity less than
        target_velocity
        """
        new_status = py_trees.common.Status.RUNNING

        if self._type == 'vehicle':
            if CarlaDataProvider.get_velocity(self._actor) >\
