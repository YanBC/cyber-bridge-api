...
class VehicleRunTest(Criterion):
    ...
    - optional [optional]: If True, the result is not considered for
...
class InArriveRegionTest(Criterion):
    ...
    """
    The test is a success if the actor is within a given radius of a specified
    region

    Important parameters:
    - actor: CARLA actor to be used for this test
    - x, y, radius: Position (x,y) and radius (in meters) used to get the
                    checked region
    - lateral_distance: the distance between the actor right edge and the
                        position
    """

    def __init__(self, actor, x, y, radius=10, lateral_distance=0.3,
                 name="InArriveRegionTest"):
        """
        """
        ...
        self._x = self._calc_close_to_sidewalk_ref_point(x, y).x
        # pylint: disable=invalid-name
        self._y = self._calc_close_to_sidewalk_ref_point(x, y).y
    ...
    def _calc_close_to_sidewalk_ref_point(self, x, y):
        ...
        offset_location = carla.Location(0.5 * lane_width *
                                         math.cos(math.radians(offset_angel)),
                                         0.5 * lane_width *
                                         math.sin(math.radians(offset_angel)))
    ...
    def update(self):
        ...
        lateral_diff = (location.x - self._x) * rv.x +\
                       (location.y - self._y) * rv.y

        if isinstance(self._actor, (carla.Vehicle, carla.Walker)):
            actor_extent_y = self._actor.bounding_box.extent.y

        if self.test_status != "SUCCESS":
            xy_distance = math.sqrt(((location.x - self._x)**2) +
                                    ((location.y - self._y)**2))
            in_radius = xy_distance < self._radius
            close_to_lane = math.fabs(lateral_diff) <=\
                (actor_extent_y + self._lateral_distance)
            not_in_sidewalk = math.fabs(lateral_diff) > actor_extent_y
            self.actual_value = (round(xy_distance, 2),
                                 round(math.fabs(math.fabs(lateral_diff) -
                                                 actor_extent_y), 2))
            actor_keep_stop =\
                CarlaDataProvider.get_velocity(self._actor) <= 0.001

            if actor_keep_stop and \
               in_radius and close_to_lane and not_in_sidewalk:
                route_completion_event =\
                    TrafficEvent(event_type=TrafficEventType.ROUTE_COMPLETED)
                route_completion_event.set_message("Destination "
...
class MaxSpeedLimitTest(Criterion):
    ...
    def __init__(self, actor, max_velocity_allowed, optional=False,
                 name="CheckMaximumVelocity"):
        """
        Setup actor and maximum allowed velovity
        """
        self.greater_than_speed_lower_limit = False
        super(MaxSpeedLimitTest, self).\
            __init__(name, actor, round(max_velocity_allowed, 2), None,
                     optional)
...
class ArriveAtLocationTest(Criterion):
    ...
    def __init__(self, actor, destination, min_distance=0.01,
                 allow_exceed=True, optional=False,
                 name="CheckArriveAtLocationTest"):
        """
        Setup actor and maximum allowed velovity
        """
        ...
        self._actor = actor
        super(ArriveAtLocationTest, self).\
            __init__(name, self._actor, self._distance, None, optional)
    ...
    def update(self):
        ...
        extend_x = self.actor.bounding_box.extent.x \
            if self.actor.bounding_box.extent is not None else 0
...
class StandStillTimeTest(Criterion):
    ...
    Important parameters:
    - actor: CARLA ego vehicle
    - other_actor: CARLA actor to execute the behavior
    - name: Name of the condition
    - duration: Duration of the behavior in seconds, true when less than
                expected duration.

    The condition terminates with SUCCESS, when the actor does not move
    """

    def __init__(self, actor, other_actor=None, duration=0, optional=False,
                 name="CheckStandStillTimeTest"):
        """
        Setup actor
        """
        ...
        super(StandStillTimeTest, self).__init__(name, actor, self._duration,
                                                 None, optional)
    ...
    def initialise(self):
        ...
        if self._other_actor is not None:
            self._other_actor_last_location =\
                CarlaDataProvider.get_location(self._other_actor)
        else:
            self._actor_last_location =\
    ...
    def update(self):
        ...
        ego_wp = map.get_waypoint(
            CarlaDataProvider.get_location(self._actor),
            project_to_road=False,
            lane_type=(carla.LaneType.Driving | carla.LaneType.Shoulder |
                       carla.LaneType.Sidewalk))
        other_actor_wp = map.get_waypoint(
            CarlaDataProvider.get_location(self._other_actor)) \
            if self._other_actor is not None else None
        ...
        if self._other_actor is not None:
            ...
            actor_run_distance = calculate_distance(
                CarlaDataProvider.get_location(self._actor),
                self._actor_last_location)
        ...
        if velocity <= EPSILON and self._other_actor_stay_in_safe_region:
            ...
            if self.test_status != "SUCCESS":
                # keep still before and 0 means never stop
    ...
    def terminate(self, new_status):
        ...
        self.logger.debug("%s.terminate()[%s->%s]" % (self.__class__.__name__,
                                                      self.status, new_status))
...
class StandStillDistanceTest(Criterion):
    ...
    def __init__(self, actor, destination, min_distance=0.01, optional=False,
                 name="CheckStandStillDistanceTest"):
        """
        Setup actor and maximum allowed velovity
        """
        self._destination = destination
        ...
        self._actor = actor
        super(StandStillDistanceTest, self).__init__(name, self._actor,
                                                     self._excepted_distance,
                                                     None,
                                                     optional)
    ...
    def initialise(self):
        ...
        Initialize the start time of this condition
        """
        self._extend_x = self._actor.bounding_box.extent.x
...
class NeverStopTest(Criterion):
    ...
    def __init__(self, actor, optional=False, name="CheckNeverStopTest"):
        ...
        super(NeverStopTest, self).__init__(name, self._actor, 0, None,
                                            optional)
...
class VehicleTurnSignalTest(Criterion):

    """
    This class contains an atomic test for turn signal.
    ...
    def __init__(self, actor, light_state, optional=False,
                 name="CheckVehicleTurnSignalTest"):
        """
        Setup actor and maximum allowed velovity
        """
        self._actor = actor
        self._actor_light_state = light_state
        super(VehicleTurnSignalTest, self).__init__(name, self._actor, 1, None,
                                                    optional)
    ...
    def update(self):
        ...
        if self.test_status != "SUCCESS":
            if self._actor.get_light_state() == self._actor_light_state:
                self.actual_value = 1
                self.test_status = "SUCCESS"
...
class MoveOnRunWhenMeetObsTest(Criterion):

    """
    This class contains an atomic test for max stand still time when keep
    safe distance with obstacle.
    ...
    def __init__(self, actor, other_actor,
                 optional=False,
                 name="CheckMoveOnRunWhenMeetObsTest"):
        """
        Setup actor and maximum allowed velovity
        """
        self._actor = actor
        ...
        self._is_safe = False
        super(MoveOnRunWhenMeetObsTest, self).__init__(name, self._actor,
                                                       '<= 3', None, optional)
    ...
    def initialise(self):
        ...
        if self._other_actor is not None:
            self._other_extend_x = self._other_actor.bounding_box.extent.x
        else:
            self._is_safe = True
        self._last_stand_still_time = GameTime.get_time()
        super(MoveOnRunWhenMeetObsTest, self).initialise()
    ...
    def update(self):
        ...
        if self._other_actor is not None and \
           self._max_velocity - cur_velocity > 5/3.6:  # 5km/h
            distance_vector = CarlaDataProvider.get_location(self._actor) -\
                              CarlaDataProvider.get_location(self._other_actor)
            rv = self._actor.get_transform().rotation.get_right_vector()
            lateral_diff = distance_vector.x * rv.x + distance_vector.y * rv.y
            if lateral_diff > self._extend_x + self._other_extend_x:
                self._is_safe = True

        if not self._is_safe or\
           CarlaDataProvider.get_velocity(self._actor) > EPSILON:
            self._last_stand_still_time = GameTime.get_time()

        self.actual_value = GameTime.get_time() - self._last_stand_still_time
        if self.actual_value > 3:
            self.test_status = "FAILURE"
