...
class ControlCmdSender:
    def __init__(self, bridge_host, bridge_port, channel, msgType):
        self.channel = channel
        self.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.socket.connect((bridge_host, bridge_port))

        for registerBytes in op_register(ControlCommand.DESCRIPTOR.file):
            self.socket.sendall(registerBytes)
        self.socket.sendall(op_add_writer(self.channel, msgType))

    def send(self, ctrl:carla.VehicleControl) -> bool:
        # extract info
        ...
        pbCtrl.steering_target = steer * 100
...
class ControlCmdReceiver:
    def __init__(self, bridge_host, bridge_port, channel, msgType):
        ...
        self.socket.sendall(op_add_reader(self.channel, msgType))

    def recv(self) -> carla.VehicleControl:
        pbCtrlBytes = self.socket.recv(655360)

        offset = 1
        chanSizeBytes = pbCtrlBytes[offset:offset+4]
        offset += 4

        chanSize = int.from_bytes(chanSizeBytes, byteorder='little')
        offset += chanSize

        msgSizeBytes = pbCtrlBytes[offset:offset+4]
        offset += 4
        ...
        pbCtrl = ControlCommand()
        pbCtrl.ParseFromString(msgBytes)

        carlaCtrl = carla.VehicleControl()
